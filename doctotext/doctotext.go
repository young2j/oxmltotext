...
func ExtractFromPath(path string) (string, error) {
	output, err := exec.Command("antiword", path).Output()
	if err != nil {
		return "", err
	}
...
// Then it uses the "antiword" command to extract the text from the temporary file.
...
// ExtractFromPathByTika extracts text content from a doc file specified by the given path
